...
class EMA(nn.Module):
    ...
    def save_model(self, ema_model_path):
        torch.save(self.model.state_dict(), ema_model_path)

def save_swa(train_dataloader, val_dataloader, args, validate_fn, swa, logger, epoch, best_score, model_save_path, fold):
    torch.optim.swa_utils.update_bn(train_dataloader, swa)
    validate_and_save_model(args, val_dataloader, swa, best_score, epoch, validate_fn, logger, model_save_path, fold)
